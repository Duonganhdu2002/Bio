-- Bio: thêm "phong cách" (layout/shape) cho trang public — trục độc lập với theme màu.
-- Khóa khớp `BIO_STYLE_KEYS` (components/bio/theme.ts):
-- classic | pill | square | outline | bold | editorial.

alter table public.profiles
  add column if not exists layout text not null default 'classic';

-- Cập nhật RPC trang public để trả thêm `layout` trong object `profile`.
