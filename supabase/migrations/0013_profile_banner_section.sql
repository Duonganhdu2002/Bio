-- Phân loại banner: tab Shop > "Dành cho bạn" | "Brand".

alter table public.profile_banners
  add column if not exists section text not null default 'for_you'
  check (section in ('for_you', 'brand'));

create index profile_banners_profile_section_position_idx
  on public.profile_banners (profile_id, section, position);
